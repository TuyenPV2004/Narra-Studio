  ...
  return (
    <section className="editorial-workspace">
      <header className="editorial-toolbar">
        <h3>Từ bằng chứng đến storyboard, qua các cổng duyệt</h3>
      ...
      {active === 'RESEARCH' && (
        <div className="editorial-stage-layout">
          <section className="stage-main">
            ...
            <textarea id="research-packet" rows={15} value={researchDraft} onChange={(event) => setResearchDraft(event.target.value)} placeholder="Nhập hồ sơ nghiên cứu ở định dạng Markdown" />
      ...
      {active === 'TOPIC' && (
        <section className="stage-main">
          ...
          <div className="topic-grid">{topicScores.sort((a, b) => a.recommendationRank - b.recommendationRank).map((candidate) => <article className={`topic-card ${candidate.selected ? 'selected' : ''}`} key={candidate.id}>
            ...
            <label>Tiêu đề<input value={candidate.title} onChange={(event) => setTopicDrafts((value) => ({...value, [candidate.id]: {...candidate, title: event.target.value}}))} placeholder="Nhập tiêu đề chủ đề" /></label>
            <label>Móc câu<textarea rows={2} value={candidate.hook} onChange={(event) => setTopicDrafts((value) => ({...value, [candidate.id]: {...candidate, hook: event.target.value}}))} placeholder="Nhập móc câu mở đầu" /></label>
            <label>Góc tiếp cận<textarea rows={2} value={candidate.angle} onChange={(event) => setTopicDrafts((value) => ({...value, [candidate.id]: {...candidate, angle: event.target.value}}))} placeholder="Nhập góc tiếp cận" /></label>
            <label>Lý do phù hợp<textarea rows={3} value={candidate.rationale} onChange={(event) => setTopicDrafts((value) => ({...value, [candidate.id]: {...candidate, rationale: event.target.value}}))} placeholder="Nhập lý do chủ đề phù hợp" /></label>
      ...
      {active === 'THESIS' && (
        <section className="stage-main">
          ...
          <div className="candidate-list">{workspace.thesisCandidates.map((candidate) => <article className="thesis-card" key={candidate.id}>
            <label>Phát biểu luận đề<textarea rows={3} value={thesisDrafts[candidate.id] ?? candidate.statement} onChange={(event) => setThesisDrafts((value) => ({...value, [candidate.id]: event.target.value}))} placeholder="Nhập phát biểu luận đề" /></label>
      ...
      {active === 'OUTLINE' && (
        <section className="stage-main">
          ...
          <div className="outline-list">{outlineDraft.map((section, index) => <article draggable className="outline-card" key={section.id} onDragStart={() => setDraggedId(section.id)} onDragOver={(event) => event.preventDefault()} onDrop={() => dropOutline(section.id)}>
            ...
            <div className="outline-fields"><label>Tiêu đề chương<input value={section.title} onChange={(event) => setOutlineDraft((items) => items.map((item) => item.id === section.id ? {...item, title: event.target.value} : item))} placeholder="Nhập tiêu đề chương" /></label><label>Mục tiêu<textarea rows={2} value={section.objective} onChange={(event) => setOutlineDraft((items) => items.map((item) => item.id === section.id ? {...item, objective: event.target.value} : item))} placeholder="Nhập mục tiêu của chương" /></label><label>Ghi chú nội dung<textarea rows={3} value={section.contentNotes ?? ''} onChange={(event) => setOutlineDraft((items) => items.map((item) => item.id === section.id ? {...item, contentNotes: event.target.value} : item))} placeholder="Nhập các ý nội dung cần triển khai" /></label><div className="outline-meta"><label>Thời lượng (giây)<input type="number" min="1" value={section.targetDurationSec} onChange={(event) => setOutlineDraft((items) => items.map((item) => item.id === section.id ? {...item, targetDurationSec: Number(event.target.value)} : item))} placeholder="Nhập số giây" /></label><span>Luận điểm: {section.claimIds.join(', ') || 'Không có'}</span><span>Nguồn: {section.sourceIds.join(', ') || 'Không có'}</span></div></div>
