...
contextBridge.exposeInMainWorld('api', {
  ...
  getFlowProjectInitialData: (p) => ipcRenderer.invoke('get-flow-project-initial-data', p),
  ...
  selectQuantityOnWebview: (p) => ipcRenderer.invoke('select-quantity-on-webview', p),
  selectAspectOnWebview: (p) => ipcRenderer.invoke('select-aspect-on-webview', p),
  uploadReferenceOnWebview: (p) => ipcRenderer.invoke('upload-reference-on-webview', p),
  uploadStartImageOnWebview: (p) => ipcRenderer.invoke('upload-start-image-on-webview', p),
  ...
  generateVideoStartImage: (p) => ipcRenderer.invoke('generate-video-start-image', p),
  generateVideoStartEndImage: (p) => ipcRenderer.invoke('generate-video-start-end-image', p),
  generateVideoReferenceImages: (p) => ipcRenderer.invoke('generate-video-reference-images', p),
  generateVideoEditVideo: (p) => ipcRenderer.invoke('generate-video-edit-video', p),
  generateFlowVoicePreview: (p) => ipcRenderer.invoke('generate-flow-voice-preview', p),
  ...
  resolveDownloadedVideo: (mediaName) => ipcRenderer.invoke('resolve-downloaded-video', mediaName),
  ...
  uploadImageViaWebview: (p) => ipcRenderer.invoke('upload-image-via-webview', p),
  ...
  readSkillFolder: (rootPath) => ipcRenderer.invoke('read-skill-folder', rootPath),
  ...
  workspaceImportMediaRead: (p) => ipcRenderer.invoke('workspace-import-media-read', p),
  workspaceImportRelease: (p) => ipcRenderer.invoke('workspace-import-release', p),
  createAIAgentStoryProject: (p) => ipcRenderer.invoke('create-ai-agent-story-project', p),
  ...
  teamWorkspaceActivity: (p) => ipcRenderer.invoke('team-workspace-activity', p),
  ...
  teamWorkspaceRemoveMember: (p) => ipcRenderer.invoke('team-workspace-remove-member', p),
  teamWorkspaceRename: (p) => ipcRenderer.invoke('team-workspace-rename', p),
  teamWorkspaceMemberRole: (p) => ipcRenderer.invoke('team-workspace-member-role', p),
  teamWorkspaceTransferOwner: (p) => ipcRenderer.invoke('team-workspace-transfer-owner', p),
  ...
  teamPresenceWalStatus: (p) => ipcRenderer.invoke('team-presence-wal-status', p),
  ...
  teamCanvasEpisodeUpdate: (p) => ipcRenderer.invoke('team-canvas-episode-update', p),
  teamCanvasEpisodesReorder: (p) => ipcRenderer.invoke('team-canvas-episodes-reorder', p),
  ...
  teamWorkspaceAssetList: (p) => ipcRenderer.invoke('team-workspace-asset-list', p),
  teamWorkspaceAssetUpsert: (p) => ipcRenderer.invoke('team-workspace-asset-upsert', p),
  teamWorkspaceAssetCloneRecord: (p) => ipcRenderer.invoke('team-workspace-asset-clone-record', p),
  teamWorkspaceAssetArchive: (p) => ipcRenderer.invoke('team-workspace-asset-archive', p),
  teamWorkspaceToolboxList: (p) => ipcRenderer.invoke('team-workspace-toolbox-list', p),
  teamWorkspaceToolboxUpsert: (p) => ipcRenderer.invoke('team-workspace-toolbox-upsert', p),
  teamWorkspaceToolboxDelete: (p) => ipcRenderer.invoke('team-workspace-toolbox-delete', p),
  ...
  saveUserPresets: (payload) => ipcRenderer.invoke('save-user-presets', payload),
  importUserPresetFile: () => ipcRenderer.invoke('import-user-preset-file'),
  exportUserPresetTemplate: (params) => ipcRenderer.invoke('export-user-preset-template', params),
  ...
  changeImageOutputFolder: () => ipcRenderer.invoke('change-image-output-folder'),
  ...
  concatVideosWithTransitions: (p) => ipcRenderer.invoke('concat-videos-with-transitions', p),
  // Project management
  ...
  projectsDuplicate: (id, newName) => ipcRenderer.invoke('projects:duplicate', { id, newName }),
  ...
  selectAgentCanvasMediaFiles: () => ipcRenderer.invoke('select-agent-canvas-media-files'),
  ...
  cancelDepthAnythingVideo: (p) => ipcRenderer.invoke('depth-anything-cancel', p),
  ...
  cancelVideoAudioDemux: (p) => ipcRenderer.invoke('video-audio-demux-cancel', p),
  onVideoAudioDemuxProgress: (callback) => {
    const handler = (_event, payload) => callback(payload);
    ipcRenderer.on('video-audio-demux-progress', handler);
    return () => ipcRenderer.removeListener('video-audio-demux-progress', handler);
  },
  separateVideoAudioStems: (p) => ipcRenderer.invoke('video-audio-separate-stems', p),
  cancelVideoAudioSeparation: (p) => ipcRenderer.invoke('video-audio-separate-stems-cancel', p),
  onVideoAudioSeparationProgress: (callback) => {
    const handler = (_event, payload) => callback(payload);
    ipcRenderer.on('video-audio-separation-progress', handler);
    return () => ipcRenderer.removeListener('video-audio-separation-progress', handler);
  },
  ...
  concatWithTransitions: (p) => ipcRenderer.invoke('concat-with-transitions', p),
  ...
  downloadVoiceAsset: (assetId) => ipcRenderer.invoke('download-voice-asset', { assetId }),
  ...
  prepareLocalLipSyncEngine: (p) => ipcRenderer.invoke('prepare-local-lip-sync-engine', p),
  ...
  aiAgentChatStream: (p, cb) => {
    ...
    const promise = ipcRenderer.invoke('ai-agent-chat-stream', p)
      .finally(() => ipcRenderer.removeListener('ai-agent-chat-stream', handler));
    return {
      promise,
  ...
  aiAgentPolishWorkflow: (p) => ipcRenderer.invoke('ai-agent-polish-workflow', p),
