...
- **Files allowed:** `apps/desktop/src/ui/**`, its isolated generated output ignore rule, frontend-only validation scripts, package manifests/lockfile and this plan.
...
- **Status:** `RUNTIME_VERIFIED` for all reachable source panels. Provider-neutral Text/Vision, protocol-constrained TTS and lip-sync profiles are now configured through Provider Account; retired Avis/KYC and unreferenced legacy preview helpers remain explicitly outside the active source scope
...
- **Continuation slices:** Image Editor now exposes the recovered VEO `transformImage` crop contract with media/workflow response normalization and local save. Source smoke verifies Toolbox save/insert dependency remapping and Workspace Asset-to-Canvas clone persistence; audio nodes render with `<audio>` and Note `textOutput` is visible. AI Agent and editor audio/vision actions resolve the selected capability profile without built-in model or environment-key fallback
- **Safety boundary:** Automatic Avis KYC asset creation/upload remains intentionally excluded after the authorized Avis retirement. No source route or adapter invokes the historical external upload path; no placeholder KYC module or silent network-error bypass was added
...
- **Current status:** `RUNTIME_VERIFIED`; production loader is `CUT_OVER` to source and the reachable active-capability audit is complete for the maintained source routes
- **Implementation:** `apps/desktop` production build now runs `scripts/build-source-desktop.mjs`; Vite emits `apps/desktop/dist/index.html` from `ui` while the Electron Main/preload/config inputs remain unchanged
...
- **Status:** `RUNTIME_VERIFIED`; recovered runtime files are removed from the working tree and no longer on the critical runtime path
...
- **Known manual risks:** Provider-paid success paths, Google Flow image upload/edit response, native OS folder/file dialogs and media export with real user files were not invoked by automated smoke; these are operational/manual checks, not source-runtime blockers
- **Final continuation note:** `scripts/build-source-desktop.mjs` and `scripts/build-source-renderer.mjs` invoke Vite with `--configLoader runner`; this is a build reproducibility fix only and does not alter renderer behavior. The authorized provider-neutral scope deliberately excludes the retired Avis KYC/reference upload path.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Source frontend | N/A | `apps/desktop/src/ui` | `RUNTIME_VERIFIED` | Production build, 17-page typed inventory and reachable active-capability audit PASS | 15 runtime route baselines MATCH | 163 direct adapter methods exposed by preload | None | Production runtime |
...
| AI Agent | Recovered AI Agent chunks | `pages/AIAgent` | `RUNTIME_VERIFIED` | Conversations, package, six source panels, workspace graph/revisions, Note/Image/Video/Audio node controls, Toolbox/Asset clone persistence, capability-selected Text/Vision and local audio trim verified | Dedicated baseline MATCH | Exact agent/workspace/media adapters inside the 163-method guarded source surface | Paid execution and OS dialogs are manual operational checks; retired KYC/reference path is excluded | Production runtime |
...
- **Architecture:** Added a parallel source tree under `apps/desktop/src/ui` with `app`, `components/ui`, `pages`, `services/electron-api`, `styles` and `types`. `App.tsx`, bootstrap and a typed source-only route registry compile from readable React/TypeScript
...
- **Files changed:** Root/desktop package manifests and lockfile; `.gitignore`; `scripts/check-source-frontend.mjs`; `apps/desktop/src/ui/**`; this plan
...
## Phase 13 — Provider-neutral AI runtime and Avis retirement

**User-authorized scope change:** Keep Google VEO3/Google Flow as its own application/media provider, retire Avis and its KYC/reference workflow, and allow AI Agent text providers to be configured by Base URL + API key + discovered model. This phase supersedes the earlier KYC-deferred blocker. Backend changes are limited to the provider-profile boundary required to keep API keys and outbound test requests outside the renderer; existing VEO3/Flow/CAPTCHA IPC semantics remain prohibited from change.

- **Contract:** OpenAI-compatible profiles use an API-root Base URL (for example `https://host/v1`), `GET /models` for connection/model discovery and `POST /chat/completions` for AI Agent text. Remote URLs require HTTPS; loopback development endpoints may use HTTP. Embedded URL credentials are rejected.
- **Secret handling:** API keys are sent renderer→preload→Main only on save/test, encrypted with Electron `safeStorage` before persistence, never returned to the renderer and never written to logs. Profile list responses expose only `hasApiKey` and a masked preview.
- **Application boundary:** VEO3 remains the sole application/media provider. Configurable profiles are an independent AI Agent text-provider registry; they do not inherit Google session/CAPTCHA behavior and do not claim generic image/video/audio compatibility.
- **Avis removal order:** (1) add/test generic profile runtime, (2) switch active AI Agent text calls, (3) replace active Avis-only source helpers with generic text or local/VEO3 equivalents, (4) remove source Avis/KYC references, (5) remove unreachable Avis/KYC backend/preload modules only after contract/runtime gates prove no consumer.
- **Validation:** Source/backend syntax and type checks; profile validation/encryption unit tests; static IPC guard; source UI contract; provider-account Electron smoke with sanitized profile round-trip; full 15-route production smoke; no VEO3/Flow/CAPTCHA regression.
- **Rollback condition:** API key appears in renderer responses/logs, arbitrary unsafe URL handling, model discovery changes provider state, VEO3 route/IPC regression, or any active AI Agent source action still requires Avis after the retirement checkpoint.
- **Definition of Done:** No `avis`/KYC source runtime reference; AI Agent chat uses the selected OpenAI-compatible profile; Base URL/API key test and model selection work; VEO3 remains independently functional; Avis/KYC runtime modules and configuration are removed only after reachability proof; all static/build/runtime/visual gates pass.

### Phase 13 implementation status

- **Completed:** Generic OpenAI-compatible profiles now own AI Agent text runtime. Users can save an encrypted API key, test the Base URL, discover `/models`, choose a model and activate a profile. Renderer responses expose only `hasApiKey` and a masked preview.
- **Completed:** VEO3/Google Flow remains the sole image/video application provider. Image and Video source adapters no longer branch to a second media provider. Media Library is local-only.
- **Completed:** Avis/KYC renderer consumers, preload methods, IPC handlers, provider modules and Avis-only Cloudflare upload modules were removed after source reachability and TypeScript/runtime contract checks. Local Piper remains the audio-node fallback; text-only Script/Note actions use the active generic AI profile. Video Story media analysis is explicitly unavailable until a provider-neutral media contract exists.
- **Validation note:** This phase changes the approved backend provider boundary. The VEO3/Flow/CAPTCHA/generation contracts are unchanged; backend files touched are limited to removing Avis/KYC and adding the generic profile boundary.
- **Remaining risk:** Existing user settings may still contain obsolete `avis*` keys; they are ignored by the new runtime and are not migrated or printed. All AI text/vision actions now require an active custom provider profile; no built-in model or provider fallback is selected.
- **Source IPC manifest:** Regenerated after retirement: 164 distinct direct renderer adapter methods, all exposed by preload; the guard remains a static method-name/import baseline and does not validate payload/response schemas.
- **Runtime/visual validation:** `pnpm validate` PASS; provider profile security tests PASS; source contract/UI contract PASS; Electron package smoke PASS; 15-route Electron smoke PASS. Provider Account and Media Library baselines were intentionally updated after reviewing the provider-neutral UI diff; Image Editor baseline was refreshed only for the previously observed scrollbar/viewport drift with unchanged interaction assertions.

### Phase 13 hardening follow-up — provider configuration contract

- **Status:** Completed within the authorized provider/configuration boundary; no commit or push performed.
- **Environment contract:** `.env` and `.env.example` now expose the same 4 active non-secret inference-tuning keys. Avis/KYC/Cloudflare, Ollama, EZAI/AI_API_KEY, provider credentials, the optional Google Flow credits key and six unused `NARRA_*` template keys were removed. `scripts/check-env-contract.mjs` is part of `pnpm validate` and fails on key drift or obsolete keys.
- **Provider URL safety:** Custom OpenAI-compatible profiles reject credentials, query/fragment components, endpoint paths, and obvious private/link-local/reserved IP literals. HTTPS remains required for remote hosts and HTTP remains available for local loopback providers. DNS-resolved private targets are not fully prevented because desktop users may intentionally connect to LAN hostnames; this remains an explicit residual SSRF risk.
- **Provider response safety:** Model discovery rejects malformed/non-JSON/empty model lists. Text chat now rejects non-JSON and non-text/empty response shapes for the custom OpenAI-compatible contract. This is a boundary validator, not a full provider schema validator.
- **Secret handling:** API key previews no longer expose the key prefix, and provider HTTP error text is redacted when it contains the configured key. Real keys remain in OS-encrypted profile storage and are not returned to the renderer.
- **Legacy settings:** Obsolete persisted `avis*` settings remain untouched for non-destructive compatibility and are ignored by the provider-neutral runtime; they are not logged, migrated, or exposed. Old Ollama settings are likewise ignored and no longer have a runtime consumer.
- **Legacy AI fallback removal:** The old EZAI/`AI_API_KEY` subtitle and watermark paths were replaced with the active OpenAI-compatible provider profile. These editor actions now require a configured profile with `format: openai` (vision-capable); no old EZAI endpoint or environment key remains.
- **Custom-provider-only correction:** Ollama environment variables, default model `glm-5-2`, native Ollama request schema, Ollama settings IPC and renderer readiness checks were removed. AI Agent chat, deflicker, subtitle and watermark actions now require the selected custom OpenAI-compatible profile; model choice comes only from the saved/discovered profile and no built-in model is selected.
- **Credential alias cleanup:** `ELEVENLABS_API_KEY`, `LIP_SYNC_API_KEY`, `VEO3_LIPSYNC_API_KEY`, `ITERA102_KEY_FILE`, `ITERA102_API_KEY` and `SYNC_API_KEY` were removed from runtime environment lookup and templates. TTS and lip-sync now use encrypted custom provider profiles selected per capability; persisted legacy keys are ignored and no longer returned to the renderer.
- **Capability profile configuration:** TTS and lip-sync now use the same encrypted custom provider profiles as text/vision, selected per capability in Provider Account. `ITERA102_API_KEY` and `SYNC_API_KEY` were removed from `.env`; VEO3/Flow remains independent.
- **Protocol boundary:** Profiles declare one explicit protocol: `openai-compatible` for Text/Vision, `narra-tts-v1` for the existing TTS request contract, or `sync-v2` for the existing lip-sync create/poll contract. UI capability selection is constrained by protocol, and Text/Vision/TTS/Lip-sync keep separate active-profile IDs. This avoids implying that a chat endpoint can execute TTS or lip-sync.
- **Validation follow-up (2026-08-18):** User-run `pnpm validate` PASS, including env contract, formatting, desktop/source typecheck, source IPC guard, source frontend guard, provider profile tests, UI contract tests, package resolver tests, desktop build and source build. User-run `pnpm check:frontend-contract` PASS, `pnpm test` PASS and `git diff --check` PASS with non-blocking LF→CRLF warnings only. User-run `pnpm package:electron-smoke` PASS and `pnpm smoke:electron-ui:all` PASS for all 15 production routes. Packaging emitted non-blocking warnings for deprecated transitive packages, missing optional platform packages, missing package metadata and a `jiti` bin link; no renderer/runtime failure was reported. Electron-path diff is intentionally non-empty because Phase 13 changes the provider boundary and removes Avis/KYC modules.
