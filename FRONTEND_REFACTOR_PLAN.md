...
- **Environment contract:** `.env` and `.env.example` now expose the same 10 active configuration keys. Avis/KYC/Cloudflare, Ollama, EZAI/AI_API_KEY, legacy TTS/lip-sync aliases, the optional Google Flow credits key and hardcoded env credentials were removed from both templates; `.env` contains only local defaults and non-secret runtime tuning values. `scripts/check-env-contract.mjs` is now part of `pnpm validate` and fails on key drift or obsolete provider keys.
